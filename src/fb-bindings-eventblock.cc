...
void event_block::event_notification _UV_NOTIFICATION_SIGNATURE {
	...
	if(!eb->queue()) {
            Nan::ThrowError(
            String::Concat(Isolate::GetCurrent(), Nan::New("While isc_que_events in event_notification - ").ToLocalChecked(),ERR_MSG(eb, event_block)));
        }			      
...
void
    ...
    { 
      ...
      if(!eb->queue()) {
            Nan::ThrowError(
            String::Concat(Isolate::GetCurrent(), Nan::New("While isc_que_events - ").ToLocalChecked(),ERR_MSG(eb, event_block)));
            return ;
      }
    }
...
void
    event_block::RegEvent(event_block** rootp, char *Event, Connection *aconn, isc_db_handle *db)
    {
      ...
      if(!res->cancel()){
    	    Nan::ThrowError(
        	String::Concat(Isolate::GetCurrent(), Nan::New("While cancel_events - ").ToLocalChecked(),ERR_MSG(res, event_block)));
    	    return;
      }        
...
void 
    event_block::RemoveEvent(event_block** root, char *Event)
    {
      ...
      if(eb)
      {
        ...
        if(!eb->cancel()){
        	Nan::ThrowError(
        	        	    String::Concat(Isolate::GetCurrent(), Nan::New("While cancel_events - ").ToLocalChecked(),ERR_MSG(eb, event_block)));
    	    return ;
    	}        
